					...
					<td class="search-col-item border-bottom-gap"><select
						class="search-select" name="searchCategory">
							...
							<option value="userName"
								${searchCategory == 'userName' ? 'selected' : ''}>利用者氏名</option>
							<%
							}
							%>
							<!--           <option>著者</option> -->
					...
					<tr>
						...
						<td class="col-action-cell">
							...
							<form action="${pageContext.request.contextPath}/userStatus">
								...
								<button type="submit" class="action-btn" style="width: 80%; margin: -1px; padding: 2px 8px; font-size: 13px;">詳細</button>
							</form>
						</td>
					</tr>
				</c:forEach>
	...
	<div class="pagination-buttons">
		<c:choose>
			<c:when test="${currentPage > 1}">
				<button type="button" onclick="location.href='${pageContext.request.contextPath}/reserveStatusInquiry?page=${currentPage - 1}'">◀</button>
			</c:when>
			<c:otherwise>
